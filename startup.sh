...
if [ $? != 0 ]; then
  ...
  tmux new-window -t $SESSION_NAME -n 'Res'

  tmux new-window -t $SESSION_NAME -n 'Front_Runner'
  tmux new-window -t $SESSION_NAME -n 'Auth_Runner'
  tmux new-window -t $SESSION_NAME -n 'Res_Runner'
  tmux new-window -t $SESSION_NAME -n 'Nginx'
  tmux new-window -t $SESSION_NAME -n 'git'

  # Send some commands to the main window to make sure we are in the right project
  tmux send-keys -t $SESSION_NAME:1 'cd $MNC_HOME/Frontend' C-m
  tmux send-keys -t $SESSION_NAME:2 'cd $MNC_HOME/Backend/Authentication-Service/' C-m
  tmux send-keys -t $SESSION_NAME:3 'cd $MNC_HOME/Backend/Profile-Resource-Service' C-m
  tmux send-keys -t $SESSION_NAME:4 'cd $MNC_HOME/Frontend' C-m
  tmux send-keys -t $SESSION_NAME:5 'cd $MNC_HOME/Backend/Authentication-Service' C-m
  tmux send-keys -t $SESSION_NAME:6 'cd $MNC_HOME/Backend/Profile-Resource-Service' C-m
  tmux send-keys -t $SESSION_NAME:7 'cd $MNC_HOME/Backend/Docker-Services/nginx' C-m
  tmux send-keys -t $SESSION_NAME:8 'cd $MNC_HOME' C-m

  # Unlock the vault
  tmux send-keys -t $SESSION_NAME:0 '$MNC_HOME/unsealvault.sh' C-m
